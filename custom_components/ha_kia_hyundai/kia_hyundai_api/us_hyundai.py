...
def _seat_settings_hyundai(level: SeatSettings | None) -> int:
    """Convert seat setting to Hyundai API value.
    
    Hyundai uses different seat heat state values:
    drvSeatHeatState: 0=off, 1=low, 2=med, 3=high (for heat)
    astSeatHeatState: same for passenger
    """
    if level is None:
        return 0
    ...
    # Map our SeatSettings to Hyundai's values
    if level_value == 6:  # HeatHigh
        ...
        return 1
    elif level_value == 3:  # CoolHigh (ventilation)
        return 3  # Hyundai uses same range for cooling
...
class UsHyundai:
    """Hyundai BlueLink USA API client."""
    ...
    async def login(self):
        ...
        if response_json.get("access_token"):
            ...
            _LOGGER.info("========== HYUNDAI LOGIN SUCCESS (direct auth) ==========")
            return
        
        # Check if OTP is required
        if "otpKey" in response_json or response_json.get("responseCode") == "OTP_REQUIRED":
            _LOGGER.info("========== HYUNDAI OTP REQUIRED ==========")
            ...
            _LOGGER.info("========== HYUNDAI LOGIN SUCCESS (OTP auth) ==========")
            return
            ...
            "vehicleConfig": {
                ...
                "vehicleFeature": {
                    "remoteFeature": {
                        ...
                        "steeringWheelStepLevel": "1",  # Hyundai typically only has on/off
                    },
                },
                "heatVentSeat": {
